...
Section Install
  ...
  SetRegView 64
  ReadRegStr $R2 HKCU "$R1" "Fork"
  ${If} $R2 != ""
    DeleteRegValue HKCU "$R1" "Fork"
  ${EndIf}
  ReadRegStr $R2 HKLM "$R1" "Fork"
  ${If} $R2 != ""
    DeleteRegValue HKLM "$R1" "Fork"
  ${EndIf}
  ...
  IfFileExists "$INSTDIR\Fork.exe" 0 +2
    Delete "$INSTDIR\Fork.exe"
...
Section Uninstall
  ...
  ${If} $UpdateMode <> 1
    ...
    Delete "C:\Users\Public\Desktop\Fork.lnk"
    ...
    SetShellVarContext current
    RMDir /r /REBOOTOK "$SMPROGRAMS\Fork"
    RMDir /r /REBOOTOK "$SMPROGRAMS\clash-verge"
    !insertmacro SetContext
    RMDir /r /REBOOTOK "C:\ProgramData\Microsoft\Windows\Start Menu\Programs\Fork"
    RMDir /r /REBOOTOK "C:\ProgramData\Microsoft\Windows\Start Menu\Programs\clash-verge"

    ; Clean legacy registry keys
    SetRegView 64
    DeleteRegKey HKLM "SOFTWARE\Microsoft\Windows\CurrentVersion\App Paths\Fork.exe"
    DeleteRegKey HKLM "SOFTWARE\Microsoft\Windows\CurrentVersion\App Paths\clash-verge.exe"
    DeleteRegKey HKLM "Software\Fork"
    DeleteRegKey HKLM "Software\Fork"
    DeleteRegKey HKCU "Software\Fork"
    DeleteRegKey HKCU "Software\Fork"
    DeleteRegKey HKCU "Software\Microsoft\Windows\CurrentVersion\Uninstall\ClashVerge"
    DeleteRegKey HKCU "SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\Fork"
