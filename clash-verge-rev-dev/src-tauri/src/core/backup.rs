...
impl WebDavClient {
    ...
    async fn get_client(&self, op: Operation) -> Result<reqwest_dav::Client, Error> {
        ...
        let client = reqwest_dav::ClientBuilder::new()
            .set_agent(
                // Never accept invalid TLS certs (MITM). Use system roots only.
...
pub async fn create_backup() -> Result<(String, PathBuf), Error> {
    ...
    if let Some(obj) = verge_config.as_object_mut() {
        ...
        // Never ship startup_script in backups (restore would re-enable RCE path)
        obj.remove("startup_script");
    }
