...
// Isolated from stock Fork (Program Files) so both can run side-by-side.
pub static APP_ID: &str = crate::commercial::APP_ID;
pub static BACKUP_DIR: &str = crate::commercial::BACKUP_DIR;
...
pub fn ipc_path() -> Result<PathBuf> {
    let ns = crate::commercial::IPC_SOCK_NAMESPACE;
    let file = crate::commercial::IPC_SOCK_FILE;
    ensure_mihomo_safe_dir()
        .map(|base_dir| base_dir.join(ns).join(file))
        .or_else(|| {
            app_home_dir()
                .ok()
                .map(|dir| dir.join(ns).join(file))
        })
...
pub fn ipc_path() -> Result<PathBuf> {
    // Must not share stock Fork pipe `\\.\pipe\verge-mihomo`
    Ok(PathBuf::from(crate::commercial::IPC_PIPE_NAME))
}
#[async_trait]
