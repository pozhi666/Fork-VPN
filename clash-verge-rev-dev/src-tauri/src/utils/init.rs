...
pub(super) async fn init_dns_config() -> Result<()> {
    ...
    if !dns_path.exists() {
        ...
        help::save_yaml(&dns_path, &default_dns_config, Some("# Fork DNS Config")).await?;
    }
...
async fn initialize_config_files() -> Result<()> {
        ...
        && !path.exists()
    {
        ...
        help::save_yaml(&path, &template, Some("# Fork"))
            .await
...
pub fn init_scheme() -> Result<()> {
    ...
    let (clash, _) = hkcu.create_subkey("Software\\Classes\\fork")?;
    clash.set_value("", &"Fork")?;
    clash.set_value("URL Protocol", &"Fork URL Scheme Protocol")?;
    let (default_icon, _) = hkcu.create_subkey("Software\\Classes\\fork\\DefaultIcon")?;
    default_icon.set_value("", &app_exe)?;
    let (command, _) = hkcu.create_subkey("Software\\Classes\\fork\\Shell\\Open\\Command")?;
...
pub async fn startup_script() -> Result<()> {
    ...
    // Only allow explicit local scripts under the user home / app data — never remote paths.
    let script_dir = PathBuf::from(script_path.as_str());
    if !script_dir.is_absolute() {
        return Err(anyhow::anyhow!(
            "startup_script must be an absolute path selected by the user"
        ));
    }
    let script_canon = script_dir
        .canonicalize()
        .map_err(|e| anyhow::anyhow!("startup_script path invalid: {e}"))?;
    if !script_canon.is_file() {
        return Err(anyhow::anyhow!("startup_script is not a file"));
    }
    let path_str = script_canon.to_string_lossy();
    let shell_type = if path_str.ends_with(".sh") {
        "bash"
    } else if path_str.ends_with(".ps1") || path_str.ends_with(".bat") {
        "powershell"
    } else {
        return Err(anyhow::anyhow!(
            "unsupported script extension (only .sh/.ps1/.bat): {}",
            path_str
        ));
    };

    let working_dir = script_canon
        .parent()
        .map(|p| p.to_path_buf())
        .unwrap_or_else(|| script_canon.clone());

    logging!(
        warn,
        Type::Setup,
        "running user startup_script: {}",
        path_str
    );
