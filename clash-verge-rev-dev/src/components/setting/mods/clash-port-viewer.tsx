...
import {
  ISOLATED_DEFAULT_HTTP_PORT,
  ISOLATED_DEFAULT_MIXED_PORT,
  ISOLATED_DEFAULT_SOCKS_PORT,
  ISOLATED_DEFAULT_TPROXY_PORT,
    ...
    open: () => {
      originalPortsRef.current = {
        mixedPort:
          ...
          ISOLATED_DEFAULT_MIXED_PORT,
        socksPort: verge?.verge_socks_port ?? ISOLATED_DEFAULT_SOCKS_PORT,
        socksEnabled: verge?.verge_socks_enabled ?? false,
        httpPort: verge?.verge_port ?? ISOLATED_DEFAULT_HTTP_PORT,
        httpEnabled: verge?.verge_http_enabled ?? false,
        redirPort: verge?.verge_redir_port ?? ISOLATED_DEFAULT_TPROXY_PORT - 1,
        redirEnabled: verge?.verge_redir_enabled ?? false,
        tproxyPort: verge?.verge_tproxy_port ?? ISOLATED_DEFAULT_TPROXY_PORT,
      ...
      try {
        ...
        if (inUse) {
          ...
          if (original) {
            ...
            setSocksPort(verge?.verge_socks_port ?? ISOLATED_DEFAULT_SOCKS_PORT)
            setSocksEnabled(verge?.verge_socks_enabled ?? false)
            setHttpPort(verge?.verge_port ?? ISOLATED_DEFAULT_HTTP_PORT)
