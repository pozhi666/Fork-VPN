...
const ProfilePage = () => {
  ...
  const onUpdateAll = useLockFn(async () => {
    // Skip server-managed official profile (no remote URL).
    const items = profileItems.filter(
      (e) => e.type === 'remote' && !isOfficialProfile(e),
    )
    const target = items
      .map((item) => item.uid)
  ...
  const deleteSelectedProfiles = useLockFn(async () => {
    ...
    try {
      ...
      // Delete selected profiles (official profile is protected by backend)
      for (const uid of selectedProfiles) {
        const item = profileItems.find((p) => p.uid === uid)
        if (isOfficialProfile(item)) continue
        await deleteProfile(uid)
      }
            ...
            <>
              ...
              {COMMERCIAL_MODE && (
                <IconButton
                  ...
                  color="primary"
                  title="同步官方线路"
                  disabled={commercialSyncing}
                  onClick={() => {
                    void syncOfficial().then(() => mutateProfiles())
                  }}
                >
                  <RefreshRounded />
                </IconButton>
        ...
        >
          <Box sx={{ minWidth: 0 }}>
            ...
            <Box sx={{ fontSize: 12, color: 'text.secondary' }}>
              {authSession
                ? `${authSession.username} · ${authSession.plan} · 可导入自己的订阅，也可同步官方线路`
                : '可导入订阅；登录后可同步官方线路'}
            </Box>
          </Box>
          <Button
                ...
                {profileItems.map((item) => (
                  <Grid size={{ xs: 12, sm: 6, md: 4, lg: 3 }} key={item.file}>
                    <ProfileItem
                      ...
                      onEdit={() => {
                        if (isOfficialProfile(item)) {
                          showNotice.info(
                            '官方线路请用「同步官方线路」更新，不可手动编辑',
                          )
                          return
                        }
