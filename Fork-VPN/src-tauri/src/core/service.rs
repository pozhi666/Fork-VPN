...
mod tests {
    ...
    fn detects_app_translocation_paths() {
        let path = Path::new(
            "/private/var/folders/example/T/AppTranslocation/123/d/Fork.app/Contents/MacOS/Fork",
        );
    ...
    fn extracts_app_bundle_name_from_executable_path() {
        let path = Path::new("/Applications/Fork.app/Contents/MacOS/Fork");

        assert_eq!(
            macos_app_bundle_name(path).as_deref(),
            Some(std::ffi::OsStr::new("Fork.app"))
        );
    }
    ...
    fn resolves_existing_core_path_from_install_roots() -> std::io::Result<()> {
        ...
        let core_dir = root.join("Fork.app").join("Contents").join("MacOS");
